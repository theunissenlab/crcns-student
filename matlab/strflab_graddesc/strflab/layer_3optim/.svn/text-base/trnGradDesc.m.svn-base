...
for iter=1:iterstouse
    ...
    if iter~=1
        ...
        if options.coorDesc==0
            ...
            if options.gradNorm
                ...
                x(idx) = x(idx) - (options.stepSize * d * sign(grad(idx)));
            end
        end
  ...
  if options.display > 0
    if iter==1 || mod(iter,options.display)==0
      if options.earlyStop==0
        fprintf('iter: %03d | grad: %.3f est: %.3f (%s)\n',iter, normGrad, esterr,choose(esterr < esterr_min,'D','U'));
      else
        fprintf('iter: %03d | grad: %.3f est: %.3f (%s) | stop: %.3f (%s)\n',iter, normGrad, esterr,choose(esterr < esterr_min,'D','U'), ...
